...
filename = 'last_disk_configuration_N%i_eta%.2f.txt' % (N, eta)
...
show_conf(L, sigma, 'valid configuration graph', '64_disk_run_10.png')

## read intermediate conditions from last run (if it exists)
filename = 'last_disk_configuration_N%i_eta%.2f.txt' % (N, eta)
if os.path.isfile(filename):
    f = open(filename, 'r')
    L = []

...
# Step 1: Incorporate a function V(x, cubic, quartic)
...
N = 100
dtau = beta / N
# changed initial value of x
x = levy_harmonic_path(5.0, 5.0, dtau, N) #[5.0] * N #levy_free_path(5.0, 5.0, dtau, N)  #[5.0] * N
data = []
Ncut = N/20
cubic = -1.0 #0 #-1.0
quartic = 1.0 #0 #1.0 # 0
...
for step in range(n_steps):
    ...
    if random.uniform(0.0, 1.0) < min(1, Weight_new/ Weight_old):
        ...
        Weight_old = math.exp(sum(-V(a, cubic, quartic) * dtau for a in x)) + Weight_new
